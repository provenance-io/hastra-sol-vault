 ...
 *
 * Calls `initialize_reward_config` on vault-stake with the connected wallet as signer.
 * The wallet must be the program upgrade authority (typical for localnet / direct ops).
 *
 * Usage:
 *   ANCHOR_PROVIDER_URL=http://127.0.0.1:8899 ANCHOR_WALLET=~/.config/solana/id.json \
 *   yarn ts-node scripts/vault-stake/initialize_reward_config.ts --max_reward_bps 75
 */
...
async function main() {
    ...
    console.log("=== initialize_reward_config (vault-stake) ===\n");
