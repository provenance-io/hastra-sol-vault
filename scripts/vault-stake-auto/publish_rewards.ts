...
const args = yargs(process.argv.slice(2))
    ...
    .option("stake_program", {
        type: "string",
        description: "The address of this program (vault-stake-auto)",
        required: true,
    })
    .option("rewards_mint", {
        type: "string",
    ...
    .option("mint", {
        type: "string",
        description: "Staking token mint (AUTO)",
        required: true,
    })
    ...
    .option("vault_token_account", {
        type: "string",
        description: "Pool vault ATA that holds wYLDS for the AUTO pool",
        required: true,
    })
    .parseSync();
...
const main = async () => {
    ...
    // AllowedExternalMintPrograms PDA from vault-mint (legacy single-program vs PDA list).
    ...
    console.log("Staking Mint (AUTO)", mint.toBase58());
