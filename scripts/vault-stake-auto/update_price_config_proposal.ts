...
async function main() {
    ...
    console.log("=== update_price_config Squads Proposal ===\n");
    ...
    // Build the update_price_config instruction.
    ...
    // Note: unlike initialize, no account is created so no system_program is needed.
    ...
    // Inner TransactionMessage — payerKey is vault PDA (the upgrade authority signer).
    // No rent is needed since update_price_config modifies an existing account.
    ...
    console.log(`   3. If the feed ID changed, call verify_price to refresh the stored price`);
}
