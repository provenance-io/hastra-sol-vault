/**
 * update_external_mint_programs_limit_proposal_squads_v3.ts
 *
 * Creates a Squads v3 transaction proposal to call updateExternalMintProgramsLimit
 * on the vault-mint program. This is required when the program upgrade authority
 * is a Squads v3 vault PDA.
 *
 ...
 * (or app.squads.so on mainnet).
 *
 * Usage:
 *   ANCHOR_PROVIDER_URL=https://api.devnet.solana.com \
 *   ANCHOR_WALLET=~/.config/solana/squad-member.json \
 *   yarn ts-node scripts/vault-mint/update_external_mint_programs_limit_proposal_squads_v3.ts \
 *     --multisig_pda <SQUADS_V3_MULTISIG_PDA> \
 *     --max_programs <0-255>
 *
 * Optional: --transaction_index <N> — use N as the new proposal tx index
 * (skips reading multisig account data).
 */
...
/** Authority index for the program upgrade authority (vault). */
...
async function main() {
        ...
        if (data.length < 16) {
            throw new Error(
                `Account ${msPDA.toBase58()} data length ${data.length} is too small for Squads v3 multisig layout ` +
                    ...
                    `Did you pass the vault PDA signer instead of the multisig account? ` +
    ...
    console.log("=== updateExternalMintProgramsLimit Squads v3 Proposal ===\n");
