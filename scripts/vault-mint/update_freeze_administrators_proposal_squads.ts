/**
 * update_freeze_administrators_proposal_squads.ts
 *
 * Creates a Squads v4 vault transaction proposal to call updateFreezeAdministrators
 * on vault-mint. The vault PDA must match the program upgrade authority.
 *
 * The list is replace-all: every intended freeze administrator must be included.
 *
 * Usage:
 *   ANCHOR_PROVIDER_URL=https://api.mainnet-beta.solana.com \
 *   ANCHOR_WALLET=~/.config/solana/squad-member.json \
 *   yarn ts-node scripts/vault-mint/update_freeze_administrators_proposal_squads.ts \
 *     --multisig_pda <SQUADS_V4_MULTISIG_PDA> \
 *     --freeze_administrators <PUBKEY_1>[,<PUBKEY_2>,...]
 *
 * Optional:
...
const args = yargs(process.argv.slice(2))
    ...
    .option("freeze_administrators", {
        type: "string",
        description:
            "Comma-separated list of freeze administrator public keys (replace-all, 1–5 unique)",
        required: true,
    })
    .option("program_id", {
        type: "string",
...
async function main() {
    ...
    const freezeAdministrators = parseAdministrators(
        args.freeze_administrators,
    ...
    console.log("=== update_freeze_administrators Squads Proposal (v4 SDK, vault-mint) ===\n");
    ...
    console.log(
        ...
        freezeAdministrators.map((k) => k.toBase58()).join(", ")
    );
    console.log();
    ...
    console.log(`   4. Confirm freeze_administrators on config ${configPda.toBase58()}`);
}
