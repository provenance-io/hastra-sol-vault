...
describe("vault-stake", () => {
  ...
  const parseEvents = async (sig: string) => {
    ...
    const invokeLine = tx.meta.logMessages.find((l) =>
      l.includes("invoke [1]")
    );
    const actualProgramId = invokeLine
      ...
      : program.programId;
    console.log(
      "DEBUG parseEvents: workspace programId =",
      program.programId.toBase58()
    );
    console.log(
      "DEBUG parseEvents: actualProgramId     =",
      actualProgramId.toBase58()
    );
    console.log(
      "DEBUG parseEvents: logMessages =",
    ...
    console.log(
      "DEBUG parseEvents: parsed events =",
  ...
  /** Accounts for rewards-admin instructions that mutate StakeRewardConfig. */
  const stakeRewardConfigRewardsAuthorityAccounts = () => ({
    stakeConfig: stakeConfigPda,
  ...
  const ensureShortRewardCooldownForTests = async () => {
    ...
    await program.methods
      ...
      .accountsStrict(stakeRewardConfigRewardsAuthorityAccounts())
    ...
    (async () => {
      ...
      console.table({
        ...
        "Exchange Rate (assets per share)": (() => {
          ...
          return s.length > 9 ? `${s.slice(0, -9)}.${s.slice(-9)}` : s;
        })(),
      });
    });
  };
  before(async () => {
    ...
    mintProgramVaultTokenAccountOwner = Keypair.fromSeed(
      Buffer.alloc(32, 72)
    ).publicKey;
  ...
  describe("initialize", () => {
    it("set up vault-mint deposit so user 1 has tokens to stake", async () => {
      ...
      const userVaultedTokenBalance = await getAccount(
        provider.connection,
        userVaultTokenAccount
      );
      assert.equal(
        userVaultedTokenBalance.amount,
        createBigInt(100000_000_000)
      );
    });
    ...
    it("initializes the vault config", async () => {
      ...
      assert.equal(
        config.unbondingPeriod.toNumber(),
        0,
        "unbondingPeriod deprecated field should be 0"
      );
    ...
    it("initializes price config", async () => {
      ...
      const priceConfig = await program.account.stakePriceConfig.fetch(
        stakePriceConfigPda
      );
      assert.equal(
        priceConfig.priceScale.toString(),
        TEST_PRICE_SCALE.toString()
      );
      assert.equal(priceConfig.priceMaxStaleness.toString(), "3600");
      assert.equal(
        priceConfig.priceTimestamp.toString(),
        "0",
        "price not set until verify_price is called"
      );
    });
    ...
    it("set initial price for testing via set_price_for_testing", async () => {
      ...
      assert.ok(
        priceConfig.price.toString() === TEST_PRICE_1TO1.toString(),
        "price should be set"
      );
      assert.ok(
        priceConfig.priceTimestamp.toNumber() > 0,
        "price_timestamp should be set"
      );
    });
  ...
  describe("deposit", () => {
    it("deposits at Chainlink price (1:1) and redeems correctly", async () => {
      ...
      const user1InitialVaultedBalance = (
        await getAccount(provider.connection, userVaultTokenAccount)
      ).amount;
      ...
      assert.equal(
        user1Shares,
        ONE_BIG_SHARE,
        "User 1 should receive exactly ONE_BIG_TOKEN PRIME at 1:1 price"
      );
      ...
      await transfer(
        ...
        vaultTokenAccount,
        user.publicKey,
        ONE_BIG_TOKEN * createBigInt(10_000)
      );
      ...
      const user2Shares = (
        await getAccount(provider.connection, user2MintTokenAccount)
      ).amount;
      // shares = 10_000 * ONE_BIG_TOKEN * 1e9 / 1e9 = 10_000 * ONE_BIG_TOKEN
      assert.equal(
        user2Shares,
        ONE_BIG_SHARE * createBigInt(10_000),
        "User 2 should receive 10,000 PRIME at 1:1 price"
      );

      // step 4 - user 1 redeems; expects to receive 1 wYLDS per PRIME (1:1 price)
      await program.methods
        .redeem(new BN(user1Shares.toString()))
        .accountsStrict({
          stakeConfig: stakeConfigPda,
          vaultTokenAccount: vaultTokenAccount,
          stakeVaultTokenAccountConfig: stakeVaultTokenAccountConfigPda,
    ...
    it("user 2 redeems", async () => {
      ...
      const user2MintTokenBefore = (
        await getAccount(provider.connection, user2MintTokenAccount)
      ).amount;
      const user2VaultBalanceBefore = (
        await getAccount(provider.connection, user2VaultTokenAccount)
      ).amount;

      // With Chainlink 1:1 price, user 2 has ONE_BIG_SHARE * 10_000 PRIME from the previous test
      assert.equal(
        user2MintTokenBefore,
        ONE_BIG_SHARE * createBigInt(10_000),
        "User 2 should have 10,000 PRIME"
      );
      assert.equal(
        user2VaultBalanceBefore,
        BIG_ZERO,
        "User 2 should not have any vault tokens"
      );

      await program.methods
        .redeem(new BN(user2MintTokenBefore.toString()))
        .accountsStrict({
          stakeConfig: stakeConfigPda,
          vaultTokenAccount: vaultTokenAccount,
          stakeVaultTokenAccountConfig: stakeVaultTokenAccountConfigPda,
      ...
      const user2MintTokenAfter = (
        await getAccount(provider.connection, user2MintTokenAccount)
      ).amount;
      const user2VaultBalanceAfter = (
        await getAccount(provider.connection, user2VaultTokenAccount)
      ).amount;

      // At 1:1 price: wYLDS_returned = shares * price / price_scale = shares (1:1)
      assert.equal(
        user2VaultBalanceAfter,
        user2MintTokenBefore,
        "User 2 should receive exactly their share count in wYLDS at 1:1 price"
      );
      assert.equal(
        vaultBalanceAfter,
        vaultBalanceBefore - user2VaultBalanceAfter,
        "Vault balance should reflect all redeems"
      );
      assert.equal(
        user2MintTokenAfter,
        BIG_ZERO,
        "User should not have staked tokens after redeem"
      );
    });
    ...
    it("handles multiple deposits correctly", async () => {
      ...
      const sharesAfterSecond = (
        await getAccount(provider.connection, userMintTokenAccount)
      ).amount;

      assert.ok(
        sharesAfterSecond > sharesAfterFirst,
        "Shares after second deposit should be greater than after first"
      );
    });
  ...
  describe("redeem", () => {
    it("redeems a partial amount immediately (no waiting)", async () => {
      ...
      const mintBalanceBefore = (
        await getAccount(provider.connection, userMintTokenAccount)
      ).amount;
      const vaultBalanceBefore = (
        await getAccount(provider.connection, vaultTokenAccount)
      ).amount;
      const userVaultBalanceBefore = (
        await getAccount(provider.connection, userVaultTokenAccount)
      ).amount;

      assert.ok(
        mintBalanceBefore >= BigInt(redeemAmount.toNumber()),
        "User must have enough PRIME to redeem"
      );
      ...
      assert.ok(
        userVaultBalanceAfter > userVaultBalanceBefore,
        "User should receive wYLDS"
      );
    });
    ...
    it("redeems full balance in one call", async () => {
      ...
      if (mintBalance === BigInt(0)) {
        assert.fail(
          "Test precondition violated: expected non-zero mint balance before redeeming full balance"
        );
      }

      await program.methods
        .redeem(new BN(mintBalance.toString()))
        .accountsStrict({
          stakeConfig: stakeConfigPda,
          vaultTokenAccount: vaultTokenAccount,
          stakeVaultTokenAccountConfig: stakeVaultTokenAccountConfigPda,
  ...
  describe("rewards", () => {
    ...
    after(async () => {
      ...
      const info = await provider.connection.getAccountInfo(
        stakeRewardConfigPda
      );
      if (info) {
        await program.methods
          .updateRewardPeriodSeconds(
            STAKE_REWARD_CONFIG_DEFAULTS.rewardPeriodSeconds
          )
          .accountsStrict(stakeRewardConfigRewardsAuthorityAccounts())
          .signers([rewardsAdmin])
          .rpc();
    ...
    it("publish rewards", async () => {
      ...
      assert.equal(
        vaultBalanceAfter,
        expectedTotalAssets,
        "Vault balance should increase by reward amount"
      );
    ...
    it("publish reward multiples", async () => {
      ...
      const reward1 = await program.account.rewardPublicationRecord.fetch(
        rewardsRecordPda1
      );
      const reward2 = await program.account.rewardPublicationRecord.fetch(
        rewardsRecordPda2
      );

      assert.equal(
        reward1.amount.toString(),
        amount.toString(),
        "First reward amount should match"
      );
      assert.equal(
        reward2.amount.toString(),
        amount.toString(),
        "Second reward amount should match"
      );
      assert.equal(
        reward2.id,
        publishRewardsId,
        "Second reward id should match current reward id"
      );
    });
  ...
  describe("StakeRewardConfig", () => {
    ...
    describe("account state verification", () => {
      it("lazy-init config has correct default state", async () => {
        ...
        const config = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        assert.equal(
          config.maxRewardBps.toNumber(),
          75,
          "maxRewardBps must be 75 (0.75%)"
        );
        assert.equal(
      ...
      it("publish_rewards does not mutate config state (init_if_needed is idempotent)", async () => {
        ...
        const configBefore = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        const totalAssets = (
          await getAccount(provider.connection, vaultTokenAccount)
        ...
        const rewardsRecordPda = makeRewardsRecordPda(
          ++publishRewardsId,
          safeAmount
        );
        await program.methods
          ...
          .signers([rewardsAdmin])
          .rpc();
        const configAfter = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        assert.equal(
          configAfter.bump,
          configBefore.bump,
          "bump must not change across publish_rewards calls"
        );
        assert.equal(
      ...
      it("migrate_reward_config can be invoked more than once", async () => {
        ...
        await program.methods
          .migrateRewardConfig(
            new BN(1_000),
            d.maxPeriodRewards,
            d.rewardPeriodSeconds,
            d.maxTotalRewards
          )
          .accountsStrict(
            migrateRewardConfigAccounts(provider.wallet.publicKey)
          )
          .rpc();
        const mid = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        ...
        await program.methods
          ...
          .rpc();
        const after = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
      ...
      it("non-upgrade-authority cannot call migrate_reward_config", async () => {
        ...
        try {
          await program.methods
            ...
            .rpc();
          assert.fail(
            "Should have thrown — only the program upgrade authority may update reward config"
          );
        } catch (err) {
          expect(err).to.exist;
        }
    ...
    describe("default 0.75% cap enforcement", () => {
      it("rejects reward above 0.75% of total_assets with RewardExceedsMaxDelta", async () => {
        ...
        const overCapAmount =
          (totalAssets * BigInt(76)) / BigInt(10_000) + BigInt(1); // just over 0.75%
        const rewardsRecordPda = makeRewardsRecordPda(
          ++publishRewardsId,
          overCapAmount
        );
    ...
    describe("update_max_reward_bps", () => {
      it("rewards admin can raise cap: emits event and updates stored state", async () => {
        const configBefore = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        ...
        assert.equal(
          configAfter.maxRewardBps.toNumber(),
          newBps,
          "stored maxRewardBps must reflect new value"
        );
        assert.equal(
          configAfter.bump,
          configBefore.bump,
          "bump must not change on update"
        );
        ...
        assert.equal(
          (event.data.oldBps as BN).toNumber(),
          oldBps,
          "event old_bps must reflect previous stored value"
        );
        assert.equal(
          (event.data.newBps as BN).toNumber(),
          newBps,
          "event new_bps must match update argument"
        );
      });
      ...
      it("raised cap (50%) allows reward between old and new cap: 30% reward succeeds", async () => {
        ...
        const configCheck = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        assert.equal(
          configCheck.maxRewardBps.toNumber(),
          5_000,
          "precondition: cap should be 50%"
        );
      ...
      it("rewards admin can lower cap: stored state and event both reflect change", async () => {
        const configBefore = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        const oldBps = configBefore.maxRewardBps.toNumber();
        ...
        assert.equal(
          configAfter.maxRewardBps.toNumber(),
          newBps,
          "stored maxRewardBps must reflect lowered value"
        );
        ...
        assert.isDefined(
          event,
          "MaxRewardBpsUpdated event must be emitted on lower"
        );
        assert.equal(
          (event.data.oldBps as BN).toNumber(),
          oldBps,
          "event old_bps must be previous cap"
        );
        assert.equal(
          (event.data.newBps as BN).toNumber(),
          newBps,
          "event new_bps must be new cap"
        );
      });
      ...
      it("lowered cap re-enforces limit: reward above new cap is rejected", async () => {
        ...
        assert.equal(
          configCheck.maxRewardBps.toNumber(),
          2_000,
          "precondition: cap should be restored to 20%"
        );
        ...
        try {
          await program.methods
            ...
            .rpc();
          assert.fail(
            "Should have thrown RewardExceedsMaxDelta after restoring 20% cap"
          );
        } catch (err) {
          expect(err.toString()).to.include("RewardExceedsMaxDelta");
        }
      ...
      it("non-rewards-admin cannot update max_reward_bps", async () => {
        try {
          await program.methods
    ...
    describe("period, cooldown, and lifetime caps", () => {
      ...
      it("update_max_period_rewards updates state and emits event", async () => {
        const cfgBefore = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        ...
        const cfgAfter = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        assert.equal(
        ...
        assert.isDefined(
          event,
          "MaxPeriodRewardsUpdated event must be emitted"
        );
        assert.equal(
          (event.data.oldValue as BN).toString(),
          oldValue.toString()
        );
        assert.equal(
          (event.data.newValue as BN).toString(),
          newValue.toString()
        );
      });

      it("non-rewards-admin cannot call update_max_period_rewards", async () => {
        try {
          await program.methods
            ...
            .rpc();
          assert.fail(
            "Should have thrown — only rewards admin can update max_period_rewards"
          );
        } catch (err) {
          expect(err).to.exist;
        }
      ...
      it("update_reward_period_seconds updates state and emits event", async () => {
        ...
        const event = events.find(
          (e) => e.name === "rewardPeriodSecondsUpdated"
        );
        assert.isDefined(
          event,
          "RewardPeriodSecondsUpdated event must be emitted"
        );
        assert.equal((event.data.oldValue as BN).toNumber(), oldValue);
        assert.equal((event.data.newValue as BN).toNumber(), newValue);
      });

      it("non-rewards-admin cannot call update_reward_period_seconds", async () => {
        try {
          await program.methods
            ...
            .rpc();
          assert.fail(
            "Should have thrown — only rewards admin can update reward_period_seconds"
          );
        } catch (err) {
          expect(err).to.exist;
        }
      ...
      it("update_max_total_rewards updates state and emits event", async () => {
        ...
        const distributed = new BN(
          cfgBefore.totalRewardsDistributed.toString()
        );
        const oldValue = new BN(cfgBefore.maxTotalRewards.toString());
        const candidate = oldValue.add(new BN(999_999));
        const newValue = candidate.gt(distributed)
          ? candidate
          : distributed.add(new BN(1));
      ...
      it("non-rewards-admin cannot call update_max_total_rewards", async () => {
        const cfgBefore = await program.account.stakeRewardConfig.fetch(
          stakeRewardConfigPda
        );
        const distributed = new BN(
        ...
        try {
          await program.methods
            ...
            .rpc();
          assert.fail(
            "Should have thrown — only rewards admin can update max_total_rewards"
          );
        } catch (err) {
          expect(err).to.exist;
        }
      ...
      it("enforces cooldown between consecutive publishes", async () => {
        ...
        const firstRewardRecordPda = makeRewardsRecordPda(
          ++publishRewardsId,
          BigInt(1)
        );
        await program.methods
        ...
        const secondRewardRecordPda = makeRewardsRecordPda(
          ++publishRewardsId,
          BigInt(1)
        );
        try {
  ...
  describe("updateability", () => {
    ...
    it("allows freeze admin update by upgrade authority", async () => {
      ...
      const config = await program.account.stakeConfig.fetch(stakeConfigPda);
      const freezeAdmins = config.freezeAdministrators.map((pk) =>
        pk.toBase58()
      );
      assert.includeMembers(freezeAdmins, [
        freezeAdmin.publicKey.toBase58(),
        addFreezeAdmin.publicKey.toBase58(),
      ]);
    });
    ...
    it("allows rewards admin update by upgrade authority", async () => {
      ...
      //fetch config and verify
      const config = await program.account.stakeConfig.fetch(stakeConfigPda);
      const rewardsAdmins = config.rewardsAdministrators.map((pk) =>
        pk.toBase58()
      );
      assert.includeMembers(rewardsAdmins, [
        rewardsAdmin.publicKey.toBase58(),
        addRewardsAdmin.publicKey.toBase58(),
      ]);
    });
  ...
  describe("overflow deposits", () => {
    before(async () => {
      ...
      const [mintProgramMintAuthorityPda] =
      ...
      const user2VaultedTokenAccount = await getAccount(
        provider.connection,
        user2VaultTokenAccount
      );
      assert.isTrue(
        user2VaultedTokenAccount.amount > hundredBillsLarge,
        "User 2 has more than 100 billion vaulted tokens"
      );
    });
    after(async () => {
    ...
    it("billion dollar deposit works", async () => {
      const userVaultedTokenBalanceBefore = await getAccount(
        provider.connection,
        user2VaultTokenAccount
      );
      await program.methods
